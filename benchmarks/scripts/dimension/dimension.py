...
ndpu = 2524
train_size_in_features = int(1e5) * 256 * 16
test_size_in_features = int(1e5) * 256 * 16
n_features = train_size_in_features + test_size_in_features
...
print(f"number of features : {train_size_in_features}")
data_size = (train_size_in_features + 1) * 4
print(f"data size = {size(data_size)}")

for i_n_dim, n_dim in enumerate(n_dim_set):
    ##################################################
    #                   DATA GEN                     #
    ##################################################

    n_points = n_features // n_dim
    npoints_per_dpu = 2 * (train_size_in_features // (2 * ndpu * n_dim))
    train_size = npoints_per_dpu * ndpu

    X, y = make_classification(n_samples=n_points, n_features=n_dim, n_informative=min(4, n_dim), n_redundant=max(0,min(4, n_dim-min(4, n_dim))),
    ...
    df.index.rename("n_dim")

    df.to_csv("dimension_small.csv")
